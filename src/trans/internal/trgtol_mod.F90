...
#ifdef COLLECT_DATAPATTERN
! Data structure to store data patterns for ZCOMMBUFS, format:
!   Destination PROC (1),
!   ZCOMMBUFS Segment Start Index (2), ZCOMMBUFS Segment End Index (3),
!   Data Source Array (4),
!   Data Source Array 1st Dimension Start Index (5),
!   Data Source Array 1st Dimension End Index (6),
!   Data Source Array 2nd Dimension Index (7),
!   Data Source Array 3rd Dimension Index (8),
!   Data Source Array 4th Dimension Index (9)
! Data Source Array:
...
!   ZCOMMBUFR Segment Stride (4),
!   Data Destination Array (5),
!   Data Destination Array 1st Dimension Start Index (6),
!   Data Destination Array 1st Dimension End Index (7),
!   Data Destination Array 2nd Dimension Index (8)
! Data Destination Array:
!   0 - PGLAT
! Note: ZCOMMBUFR has a stride of ILEN
...
ALLOCATE(IPATTERNS(9,1000000))
ALLOCATE(IPATTERNR(8,1000000))
#endif
...
IF (NTRANS_SYNC_LEVEL <= 0) THEN
   !  Receive loop.........................................................
#ifdef COLLECT_DATAPATTERN
   CALL DEBUG(OPROC,"TRGTOL",__LINE__,"KNRECV",KNRECV)
#endif
   DO INR=1,KNRECV
      IRECV=KRECV(INR)
#ifdef COLLECT_DATAPATTERN
      CALL DEBUG(OPROC,"TRGTOL",__LINE__,"KSOURCE",NPRCIDS(IRECV))
      ! first 2 stores IRECV_FLD_START and IRECV_FLD_END
      CALL DEBUG(OPROC,"TRGTOL",__LINE__,"KRECVTOT",KRECVTOT(IRECV)+2) ! SIZE(ZCOMBUFR(-1:KRECVTOT(IRECV),INR))
#endif
...
#ifdef COLLECT_DATAPATTERN
CALL DEBUG(OPROC,"TRGTOL",__LINE__,"KNSEND",KNSEND)
#endif
DO INS=1,KNSEND
  ISEND=KSEND(INS)
  IPOS=IPOSPLUS(INS)
  ...
  DO JJ=ISEND_FLD_START,ISEND_FLD_END
    ...
    DO JBLK=1,NGPBLKS
      ...
      IF(IFIRST > 0) THEN
        ILAST = KGPTRSEND(2,JBLK,ISETW(INS))
#ifdef COLLECT_DATAPATTERN
        IOFFSET = (JJ-ISEND_FLD_START)*IPOS+IJPOS(JBLK,INS)-IFIRST+1
#endif
        IF(LLINDER) THEN
          ...
          ENDDO
#else
          ZCOMBUFS(IOFFSET+IFIRST:IOFFSET+ILAST,INS) = PGP(IFIRST:ILAST,KPTRGP(IFLDT),JBLK)
#endif
#ifdef COLLECT_DATAPATTERN
          IENDOFPATTERNS = IENDOFPATTERNS+1
          IPATTERNS(1, IENDOFPATTERNS) = NPRCIDS(ISEND)
          IPATTERNS(2, IENDOFPATTERNS) = IOFFSET+IFIRST
          IPATTERNS(3, IENDOFPATTERNS) = IOFFSET+ILAST
          IPATTERNS(4, IENDOFPATTERNS) = 0
          IPATTERNS(5, IENDOFPATTERNS) = IFIRST
          IPATTERNS(6, IENDOFPATTERNS) = ILAST
          IPATTERNS(7, IENDOFPATTERNS) = KPTRGP(IFLDT)
          IPATTERNS(8, IENDOFPATTERNS) = JBLK
#endif
        ELSE
          IF(LLPGPONLY) THEN
#ifndef BLOCK_PACKING
            ...
            ENDDO
#else
            ZCOMBUFS(IOFFSET+IFIRST:IOFFSET+ILAST,INS) = PGP(IFIRST:ILAST,IFLDT,JBLK)
#endif
#ifdef COLLECT_DATAPATTERN
            IENDOFPATTERNS = IENDOFPATTERNS+1
            IPATTERNS(1, IENDOFPATTERNS) = NPRCIDS(ISEND)
            ...
            IPATTERNS(5, IENDOFPATTERNS) = IFIRST
            IPATTERNS(6, IENDOFPATTERNS) = ILAST
            IPATTERNS(7, IENDOFPATTERNS) = IFLDT
            IPATTERNS(8, IENDOFPATTERNS) = JBLK
#endif
          ELSEIF(LLUV(IFLDT)) THEN
            ...
            ENDDO
#else
            ZCOMBUFS(IOFFSET+IFIRST:IOFFSET+ILAST,INS) = PGPUV(IFIRST:ILAST,IUVLEVS(IFLDT),IUVPARS(IFLDT),JBLK)
#endif
#ifdef COLLECT_DATAPATTERN
            IENDOFPATTERNS = IENDOFPATTERNS+1
            IPATTERNS(1, IENDOFPATTERNS) = NPRCIDS(ISEND)
            IPATTERNS(2, IENDOFPATTERNS) = IOFFSET+IFIRST
            IPATTERNS(3, IENDOFPATTERNS) = IOFFSET+ILAST
            IPATTERNS(4, IENDOFPATTERNS) = 2
            IPATTERNS(5, IENDOFPATTERNS) = IFIRST
            IPATTERNS(6, IENDOFPATTERNS) = ILAST
            IPATTERNS(7, IENDOFPATTERNS) = IUVLEVS(IFLDT)
            IPATTERNS(8, IENDOFPATTERNS) = IUVPARS(IFLDT)
            IPATTERNS(9, IENDOFPATTERNS) = JBLK
#endif
          ELSEIF(LLGP2(IFLDT)) THEN
            ...
            ENDDO
#else
            ZCOMBUFS(IOFFSET+IFIRST:IOFFSET+ILAST,INS) = PGP2(IFIRST:ILAST,IGP2PARS(IFLDT),JBLK)
#endif
#ifdef COLLECT_DATAPATTERN
            IENDOFPATTERNS = IENDOFPATTERNS+1
            IPATTERNS(1, IENDOFPATTERNS) = NPRCIDS(ISEND)
            IPATTERNS(2, IENDOFPATTERNS) = IOFFSET+IFIRST
            IPATTERNS(3, IENDOFPATTERNS) = IOFFSET+ILAST
            IPATTERNS(4, IENDOFPATTERNS) = 1
            IPATTERNS(5, IENDOFPATTERNS) = IFIRST
            IPATTERNS(6, IENDOFPATTERNS) = ILAST
            IPATTERNS(7, IENDOFPATTERNS) = IGP2PARS(IFLDT)
            IPATTERNS(8, IENDOFPATTERNS) = JBLK
#endif
          ELSEIF(LLGP3A(IFLDT)) THEN
            ...
            ENDDO
#else
            ZCOMBUFS(IOFFSET+IFIRST:IOFFSET+ILAST,INS) = PGP3A(IFIRST:ILAST,IGP3ALEVS(IFLDT),IGP3APARS(IFLDT),JBLK)
#endif
#ifdef COLLECT_DATAPATTERN
            IENDOFPATTERNS = IENDOFPATTERNS+1
            IPATTERNS(1, IENDOFPATTERNS) = NPRCIDS(ISEND)
            IPATTERNS(2, IENDOFPATTERNS) = IOFFSET+IFIRST
            IPATTERNS(3, IENDOFPATTERNS) = IOFFSET+ILAST
            IPATTERNS(4, IENDOFPATTERNS) = 3
            IPATTERNS(5, IENDOFPATTERNS) = IFIRST
            IPATTERNS(6, IENDOFPATTERNS) = ILAST
            IPATTERNS(7, IENDOFPATTERNS) = IGP3ALEVS(IFLDT)
            IPATTERNS(8, IENDOFPATTERNS) = IGP3APARS(IFLDT)
            IPATTERNS(9, IENDOFPATTERNS) = JBLK
#endif
          ELSEIF(LLGP3B(IFLDT)) THEN
            ...
            ENDDO
#else
            ZCOMBUFS(IOFFSET+IFIRST:IOFFSET+ILAST,INS) = PGP3B(IFIRST:ILAST,IGP3BLEVS(IFLDT),IGP3BPARS(IFLDT),JBLK)
#endif
#ifdef COLLECT_DATAPATTERN
            IENDOFPATTERNS = IENDOFPATTERNS+1
            IPATTERNS(1, IENDOFPATTERNS) = NPRCIDS(ISEND)
            IPATTERNS(2, IENDOFPATTERNS) = IOFFSET+IFIRST
            IPATTERNS(3, IENDOFPATTERNS) = IOFFSET+ILAST
            IPATTERNS(4, IENDOFPATTERNS) = 4
            IPATTERNS(5, IENDOFPATTERNS) = IFIRST
            IPATTERNS(6, IENDOFPATTERNS) = ILAST
            IPATTERNS(7, IENDOFPATTERNS) = IGP3BLEVS(IFLDT)
            IPATTERNS(8, IENDOFPATTERNS) = IGP3BPARS(IFLDT)
            IPATTERNS(9, IENDOFPATTERNS) = JBLK
#endif
          ENDIF
        ENDIF
...
#ifdef COLLECT_DATAPATTERN
  CALL DEBUG(OPROC,"TRGTOL",__LINE__,"KDEST",NPRCIDS(ISEND))
  ! first 2 stores ISEND_FLD_START and ISEND_FLD_END
  CALL DEBUG(OPROC,"TRGTOL",__LINE__,"KSENDTOT",KSENDTOT(ISEND)+2) ! SIZE(ZCOMBUFS(-1:KSENDTOT(ISEND),INS)))
#endif
...
!$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(JL,II,JFLD)
  DO JL=1,ILEN
    ...
    ENDDO
#else
    PGLAT(IRECV_FLD_START:IRECV_FLD_END,II) = &
        & ZCOMBUFR(JL:(JL+(IRECV_FLD_END-IRECV_FLD_START)*ILEN):ILEN,INR)
#endif
#ifdef COLLECT_DATAPATTERN
    IENDOFPATTERNR = IENDOFPATTERNR+1
    IPATTERNR(1,IENDOFPATTERNR) = NPRCIDS(IRECV)
    IPATTERNR(2,IENDOFPATTERNR) = JL
    IPATTERNR(3,IENDOFPATTERNR) = JL+(IRECV_FLD_END-IRECV_FLD_START)*ILEN
    IPATTERNR(4,IENDOFPATTERNR) = ILEN
    IPATTERNR(5,IENDOFPATTERNR) = 0
    IPATTERNR(6,IENDOFPATTERNR) = IRECV_FLD_START
    IPATTERNR(7,IENDOFPATTERNR) = IRECV_FLD_END
    IPATTERNR(8,IENDOFPATTERNR) = II
#endif
  ENDDO
!$OMP END PARALLEL DO
...
IF (LLPGPONLY) THEN
  CALL DEBUG(OPROC,"TRGTOL",__LINE__,"PGP_SIZE_1",SIZE(PGP,1))
  CALL DEBUG(OPROC,"TRGTOL",__LINE__,"PGP_SIZE_2",SIZE(PGP,2))
  CALL DEBUG(OPROC,"TRGTOL",__LINE__,"PGP_SIZE_3",SIZE(PGP,3))
ENDIF
IF (LLPGP2) THEN
  CALL DEBUG(OPROC,"TRGTOL",__LINE__,"PGP2_SIZE_1",SIZE(PGP2,1))
  CALL DEBUG(OPROC,"TRGTOL",__LINE__,"PGP2_SIZE_2",SIZE(PGP2,2))
  CALL DEBUG(OPROC,"TRGTOL",__LINE__,"PGP2_SIZE_3",SIZE(PGP2,3))
ENDIF
IF (LLPGPUV) THEN
  CALL DEBUG(OPROC,"TRGTOL",__LINE__,"PGPUV_SIZE_1",SIZE(PGPUV,1))
  CALL DEBUG(OPROC,"TRGTOL",__LINE__,"PGPUV_SIZE_2",SIZE(PGPUV,2))
  CALL DEBUG(OPROC,"TRGTOL",__LINE__,"PGPUV_SIZE_3",SIZE(PGPUV,3))
  CALL DEBUG(OPROC,"TRGTOL",__LINE__,"PGPUV_SIZE_4",SIZE(PGPUV,4))
ENDIF
IF (LLPGP3A) THEN
  CALL DEBUG(OPROC,"TRGTOL",__LINE__,"PGP3A_SIZE_1",SIZE(PGP3A,1))
  CALL DEBUG(OPROC,"TRGTOL",__LINE__,"PGP3A_SIZE_2",SIZE(PGP3A,2))
  CALL DEBUG(OPROC,"TRGTOL",__LINE__,"PGP3A_SIZE_3",SIZE(PGP3A,3))
  CALL DEBUG(OPROC,"TRGTOL",__LINE__,"PGP3A_SIZE_4",SIZE(PGP3A,4))
ENDIF
IF (LLPGP3B) THEN
  CALL DEBUG(OPROC,"TRGTOL",__LINE__,"PGP3B_SIZE_1",SIZE(PGP3B,1))
  CALL DEBUG(OPROC,"TRGTOL",__LINE__,"PGP3B_SIZE_2",SIZE(PGP3B,2))
  CALL DEBUG(OPROC,"TRGTOL",__LINE__,"PGP3B_SIZE_3",SIZE(PGP3B,3))
  CALL DEBUG(OPROC,"TRGTOL",__LINE__,"PGP3B_SIZE_4",SIZE(PGP3B,4))
ENDIF
CALL DEBUG(OPROC,"TRGTOL",__LINE__,"PGLAT_SIZE_1",SIZE(PGLAT,1))
CALL DEBUG(OPROC,"TRGTOL",__LINE__,"PGLAT_SIZE_2",SIZE(PGLAT,2))

CALL DEBUG(OPROC,"TRGTOL",__LINE__,"IENDOFPATTERNS",IENDOFPATTERNS)
CALL DEBUG(OPROC,"TRGTOL",__LINE__,"IENDOFPATTERNR",IENDOFPATTERNR)
WRITE(FILENAMES,'(A,I0)') "trgtol_pattern_s.",MYPROC
WRITE(FILENAMER,'(A,I0)') "trgtol_pattern_r.",MYPROC
