...
!$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(JFLD,JL,II)
  DO JL=1,ILEN
    ...
    ENDDO
#else
    ZCOMBUFS(JL:(JL+(ISEND_FLD_END-ISEND_FLD_START)*ILEN):ILEN,INR) = &
        & PGLAT(ISEND_FLD_START:IRECV_FLD_END,II)
#endif
    ...
    DO JBLK=1,NGPBLKS
      ...
      IF(IFIRST > 0) THEN
        ...
        ELSEIF(LLGP3B(IFLDT)) THEN
          ...
          ENDDO
#else
          PGP3B(IFIRST:ILAST,IGP3BLEVS(IFLDT),IGP3BPARS(IFLDT),JBLK) = ZCOMMBUFR(IOFFSET+IFIRST:IOFFSET+ILAST,INR)
#endif
          IENDOFPATTERNR = IENDOFPATTERNR+1
          IPATTERNR(1, IENDOFPATTERNR) = NPRCIDS(IRECV)
          IPATTERNR(2, IENDOFPATTERNR) = IOFFSET+IFIRST
