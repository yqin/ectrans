...
! a local gridpoint interpolation of the input field to the target latitudes prior to the transforms
  ...
  DO JGL=KSLO, KDGNHD
    IPROC = D%NPROCL(JGL)
    ISTN(JGL) = D%NSTAGT1B(IPROC) + D%NPNTGTB1(KMLOC,JGL)
    IGLS = 2*KDGNHD+1-JGL
    IPROCS = D%NPROCL(IGLS)
    ISTS(JGL) = D%NSTAGT1B(IPROCS) + D%NPNTGTB1(KMLOC,IGLS)
  ENDDO

  ALLOCATE( ZQX(  2*KDGNHD,KFIELDS))
  ZQX(1:KSLO,:) = 0._JPRB
  ZQX(IENDO:2*KDGNHD,:) = 0._JPRB
  DO JF=1,KFIELDS
     DO JGL=KSLO, KDGNHD
        IGLS = 2*KDGNHD+1-JGL
        ZQX(JGL,JF)=FOUBUF(ISTN(JGL)+JF*D%NLENGT0B)
        ZQX(IGLS,JF)=FOUBUF(ISTS(JGL)+JF*D%NLENGT0B)
     ENDDO
  ENDDO
  ...
  DO IGL=KSL,KDGNH
    IGLS = 2*KDGNH+1-IGL
    PCOEFS(IGL,1:KFIELDS) = ZQX(IGL,1:KFIELDS) + ZQX(IGLS,1:KFIELDS)
    PCOEFA(IGL,1:KFIELDS) = ZQX(IGL,1:KFIELDS) - ZQX(IGLS,1:KFIELDS)
  ENDDO
